...
class BoardExcel(object):
    ...
    suffix = ['开始', '结束']
    ul_keywords = ['基带设定', '接收灵敏度测试',  '上行工作电流测试']
    dl_rows = dict()
    ul_rows = dict()
    ...
    def get_bands(self):
        '''
        获取所有要测试的band,以便一次性下发TDD,FDD的band设置
        :return:
        '''
        try:
            rows = self.dl_rows['基带设定']
            ...
            startrow = rows[0] + self.jump_row
            endrow = rows[1]
            celllst = sht.range('A{}:A{}'.format(startrow, endrow)).value
            retdict={} #{'cell0':[],'cell1':[]}
            for item in celllst:
                temp = item.strip().split('_')
                thiscell = temp[2].lower()
                bandstr = temp[3].upper()
                retdict.setdefault(thiscell,[]).append(bandstr)
    ...
    def get_txatt_norm(self):
        '''
        获取TXATT的指标规范
        :return:dict {'下限': -2.0, '标准值': 0.0, '上限': 2.0}
        '''
        try:
            sht = self.wb.sheets(BoardExcel.sheets_name[0])
            rows = self.dl_rows[self.dl_keywords[1]]
            startrow = rows[0] + 2
            dt = sht.range('C{}:E{}'.format(startrow, startrow + 1)).options(dict, transpose=True).value
            return dt
        except Exception as e:
    ...
    def write_ACPR(self, **kwargs):
        ...
        try:
            ...
            value_row_base = 'H'  # 常温结果所在列
            norm_row_base = 'I'  # 常温结论所在列
            row_dict = self.get_each_cellband_row(BoardExcel.sheets_name[0], rows[0], rows[1])

            norm_dict = self.get_norm(BoardExcel.sheets_name[0], rows[0] + self.jump_row)
            ...
            for key, item in kwargs.items():
                ...
                for each in item:
                    ...
                    ret = self.set_ACPR_colusition(adj_lower, adj_upper, norm_dict)
                    norm_lst.append(ret)
    ...
    def set_ACPR_colusition(self, lower, upper, norm_dict):
        '''
        得出结论
        :return:
        '''
        try:
            ...
            for key, item in norm_dict.items():
                ...
                if '下限' in key:
                    ...
                    indicator = '<='
                if indicator:
                    e1 = str(lower) + indicator + str(value)
                    e2 = str(upper) + indicator + str(value)
    ...
    def get_sens_norm(self,cell_band):
        '''
        获取某个band的灵敏度指标范围
        :return: [下限，上限]
        '''
        try:
            shtname = BoardExcel.sheets_name[4]
            rows = self.ul_rows['接收灵敏度测试']
            ...
            normlist=[norm_list[0],norm_list[2]]
            return normlist
        except Exception as e:
            logger.error('get_sens_norm error:{}'.format(e))
            return None
    ...
    def write_EVM(self, **kwargs):
        ...
        try:
            ...
            row_dict = self.get_each_cellband_row(BoardExcel.sheets_name[0], evm_rows[0], evm_rows[1])

            norm_dict = self.get_norm(BoardExcel.sheets_name[0], evm_rows[0] + self.jump_row)
            for key, item in kwargs.items():
                evm_range = 'H' + str(row_dict[str(key).upper()])
                evm_pass_range = 'I' + str(row_dict[str(key).upper()])
                pass_lst = []
                for evm_value in item:
                    pass_lst.append(self.set_ccdf_colusition(evm_value, norm_dict))
    ...
    def write_powerspectrum(self, **kwargs):
        '''
        ...
        try:
            ...
            norm_dict = self.get_norm(BoardExcel.sheets_name[0], rows[0] + self.jump_row)
            for key, item in kwargs.items():
                ps_range = 'H' + str(row_dict[str(key).upper()])
                ps_pass_range = 'I' + str(row_dict[str(key).upper()])
                pass_lst = []
                for ps_value in item:
                    pass_lst.append(self.set_ccdf_colusition(ps_value, norm_dict))
    ...
    def write_txatt_conclusion(self):
        '''
        ...
        try:
            ...
            shtname = BoardExcel.sheets_name[0]
            sht = self.wb.sheets(shtname)
            power_list = sht.range('I{}:I{}'.format(power_row[0] + 3, power_row[1])).value
            ...
            sht.range('K{}:K{}'.format(power_row[0] + 3, power_row[1])).options(transpose=True).value = power_con_list

            ripple_list = sht.range('H{}:H{}'.format(ripple_row[0] + 3, ripple_row[1])).value
            ...
            sht.range('I{}:I{}'.format(ripple_row[0] + 3, ripple_row[1])).options(
                transpose=True).value = ripple_con_list  # 带内波动结论

            gear_list = sht.range('H{}:H{}'.format(gear_row[0] + 3, gear_row[1])).value
            ...
            sht.range('I{}:I{}'.format(gear_row[0] + 3, gear_row[1])).options(transpose=True).value = gear_con_list
    ...
    def write_DANL(self, **kwargs):
        ...
        try:
            ...
            norm_dict = self.get_norm(shtname, danl_rows[0] + self.jump_row)
            for key, item in kwargs.items():
                danl_range = 'H' + str(row_dict[str(key).upper()])
                danl_pass_range = 'I' + str(row_dict[str(key).upper()])
                pass_lst = []
                for danl_value in item:
                    pass_lst.append(self.set_ccdf_colusition(danl_value, norm_dict))
    ...
    def write_current(self, **kwargs):
        ...
        try:
            ...
            norm_dict = self.get_norm(shtname, cur_rows[0] + self.jump_row)
            for key, item in kwargs.items():
                cur_range = 'H' + str(row_dict[str(key).upper()])
                cur_pass_range = 'I' + str(row_dict[str(key).upper()])
                pass_lst = []
                for cur_value in item:
                    pass_lst.append(self.set_ccdf_colusition(cur_value, norm_dict))
    ...
    def write_sens(self, temp=1, **kwargs):
        '''
        灵敏度
        :param kwargs:
        :return:
        '''
        logger.debug('write_sens_dict={}'.format(kwargs))
        try:
            shtname = BoardExcel.sheets_name[4]
            sht = self.wb.sheets(shtname)
            cur_rows = self.ul_rows['接收灵敏度测试']
            ...
            for key, item in kwargs.items():
                row_idx = row_dict[key.upper()]
                cur_range = value_row_base + str(row_idx)
                cur_pass_range = norm_row_base + str(row_idx)
                norm_list = self.get_norm(shtname, int(row_idx))
                pass_lst = []
                for cur_value in item:
            ...
            logger.error('write_sens error:{}'.format(e))
        else:
            self.wb.save()
...
if __name__ == '__main__':
    xel = BoardExcel()
    xel.open_excel('F:/tmp/FSV/功放测试模板.xlsx')
